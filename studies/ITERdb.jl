...
function StudyITERdb(sty::ParametersStudy, act::ParametersAllActors; kw...)
    sty = sty(kw...)
    ini, _ = case_parameters(:ITER;init_from=:scalars)
...
function _run(study::StudyITERdb)
    ...
    results = pmap(item -> run_case(study, mylock, item), iterator)
    for row in results
        if !isnothing(row)
...
function run_case(study::AbstractStudy, lock::ReentrantLock, item::String)
    act = study.act
    sty = study.sty
