...
function study_common_parameters(; kw...)
    ...
    elseif name == :data_storage_policy
        return Switch{Symbol}([:separate_folders, :merged_hdf5], "-", "Data storage policy: 'separate_folders' stores each case in a separate folder, while 'merged_hdf5' merges all cases into a single HDF5 file"; default)
    else
