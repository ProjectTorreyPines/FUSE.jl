...
function ParametersInit(::Type{Val{:equilibrium}})
    ...
    equilibrium.boundary_from = Switch([:miller, :MXH_params, :rz_points], "" ,"The starting r, z boundary taken from"; default=:miller)
    equilibrium.MXH_params = Entry(Union{Nothing,Vector{<:Real}}, "", "Vector of MXH flats", default=nothing)
    equilibrium.rz_points = Entry(Union{Nothing, Vector{Vector{<:Real}}}, "m", "R_Z boundary as Vector{Vector{<:Real}}} : r = rz_points[1], z = rz_points[2]", default=nothing)
    return equilibrium
end
