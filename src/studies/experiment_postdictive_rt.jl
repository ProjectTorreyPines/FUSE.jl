...
"""
    study_parameters(::Val{:PostdictiveRT})

Runs postdictive simulations using FINN transport model for specified device and shots
"""
function study_parameters(::Val{:PostdictiveRT})
    return FUSEparameters__ParametersStudyPostdictiveRT{Real}()
end

Base.@kwdef mutable struct FUSEparameters__ParametersStudyPostdictiveRT{T<:Real} <: ParametersStudy{T}
    _parent::WeakRef = WeakRef(nothing)
    _name::Symbol = :StudyPostdictiveRT
...
mutable struct StudyPostdictiveRT{T<:Real} <: AbstractStudy
    sty::OverrideParameters{T,FUSEparameters__ParametersStudyPostdictiveRT{T}}
    act::ParametersActors
end

function StudyPostdictiveRT(sty::ParametersStudy; kw...)
    sty = OverrideParameters(sty; kw...)
    study = StudyPostdictiveRT(sty, ParametersActors())
    parallel_environment(sty.server, sty.n_workers)
    return study
end

"""
    _run(study::StudyPostdictiveRT)

Runs the PostdictiveRT study with sty settings in parallel on designated cluster
"""
function _run(study::StudyPostdictiveRT)
    sty = study.sty
    ...
    # parallel run
    println("running $(length(sty.shots)) postdictive RT simulations with $(sty.n_workers) workers on $(sty.server)")

    ProgressMeter.@showprogress map(shot -> run_postdictive_rt_case(study, shot; sty.kw_case_parameters), sty.shots)
...
"""
    run_postdictive_rt_case(study::StudyPostdictiveRT, shot::Int; kw_case_parameters::Dict{Symbol,Any})

Run a single postdictive case using FINN transport for a given device and shot
"""
function run_postdictive_rt_case(study::StudyPostdictiveRT, shot::Int; kw_case_parameters::Dict{Symbol,Any})
    sty = study.sty
    device = sty.device
    ...
    try
        ...
        run_postdictive_rt_case(device, shot; savedir, sty.reconstruction, kw_case_parameters)
...
function run_postdictive_rt_case(device::Symbol, shot::Int; kw_case_parameters::Dict{Symbol,Any}, kw...)
    dd = IMAS.dd()
    dd_exp = IMAS.dd()
    run_postdictive_rt_case!(dd, dd_exp, device, shot; kw_case_parameters, kw...)
    return (dd=dd, dd_exp=dd_exp)
end

function run_postdictive_rt_case!(
    dd::IMAS.dd,
    dd_exp::IMAS.dd,
    device::Symbol,
    shot::Int;
