    ...
    TimerOutputs.@timeit timer "init_core_sources" begin
        ...
        if init_from == :ods && !IMAS.isempty(dd1.core_sources.source)
            ...
            if isempty(dd1.ec_launchers.beam) && findfirst(:ec, dd.core_sources.source) !== missing
                @assert act.ActorSources.ec_model == :none "Init using sources from ODS requires `act.ActorSources.ec_model = :none` or data in `ods.ec_launchers.beam`"
            end
            if isempty(dd1.ic_antennas.antenna) && findfirst(:ic, dd.core_sources.source) !== missing
                @assert act.ActorSources.ic_model == :none "Init using sources from ODS requires `act.ActorSources.ic_model = :none` or data in `ods.ic_launchers.antenna`"
            end
            if isempty(dd1.lh_antennas.antenna) && findfirst(:lh, dd.core_sources.source) !== missing
                @assert act.ActorSources.lh_model == :none "Init using sources from ODS requires `act.ActorSources.lh_model = :none` or data in `ods.lh_antennas.antenna`"
            end
            if isempty(dd1.nbi.unit) && findfirst(:nbi, dd.core_sources.source) !== missing
                @assert act.ActorSources.nb_model == :none "Init using sources from ODS requires `act.ActorSources.nb_model = :none` or data in `ods.nbi.unit`"
            end
            if !isempty(dd.pellets.time_slice) && !isempty(dd.pellets.time_slice[].pellet) && findfirst(:pellet, dd.core_sources.source) !== missing
                @assert act.ActorSources.pellet_model == :none "Init using sources from ODS requires `act.ActorSources.pellet_model = :none` or data in `dd.pellets.time_slice[].pellet`"
            end
        else
