...
Base.@kwdef mutable struct FUSEparameters__requirements{T} <: ParametersInit where {T<:Real}
    ...
    power_electric_net::Entry{T} = Entry{T}("W", "Net electric power generated by the fusion power plant")
    flattop_duration::Entry{T} = Entry{T}("s", "Duration of the flattop (use Inf for steady-state)")
    ...
    tritium_breeding_ratio::Entry{T} = Entry{T}("-", "Tritium breeding ratio of the whole plant")
    ne_peaking::Entry{T} = Entry{T}("-", "Profile peaking of electron density, ne0/<ne>")
    q_pol_omp::Entry{T} = Entry{T}("W/m² ", "Poloidal heat flux at the outer midplane")
    lh_power_threshold_fraction::Entry{T} = Entry{T}("-", "Fraction of the LH power threshold")
end
