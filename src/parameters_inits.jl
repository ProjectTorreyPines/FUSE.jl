...
#TEST ORSO/JEROME
@init_params struct target{T}
    power_electric_net = Entry(T, "W", "Target net electric power generated by the fusion power plant")
    flattop_duration = Entry(T, "s", "Target duration of the flattop (use Inf for steady-state)")
    tritium_breeding_ratio = Entry(T, "-", "Target tritium breeding ratio of the whole plant")
    cost = Entry(T, "\$M", "Target total FPP cost")
end

@test_kwdef struct target2{T<:Real} <: ParametersInit
    power_electric_net = Entry(T, "W", "Target net electric power generated by the fusion power plant")
    flattop_duration = Entry(T, "s", "Target duration of the flattop (use Inf for steady-state)")
    tritium_breeding_ratio = Entry(T, "-", "Target tritium breeding ratio of the whole plant")
    cost = Entry(T, "\$M", "Target total FPP cost")
...
mutable struct ParametersInits{T} <: ParametersAllInits where {T<:Real}
    ...
    target::FUSEparameters__target{T}
end
