...
function _step(actor::ActorCXbuild; rebuild_wall::Bool=actor.par.rebuild_wall)
    ...
    build_cx!(dd.build, pr, pz, dd.equilibrium.time_slice[].global_quantities.magnetic_axis.z)
...
"""
    build_cx!(bd::IMAS.build, pr::Vector{Float64}, pz::Vector{Float64}, Z0::Float64)

Translates 1D build to 2D cross-sections starting from R and Z coordinates of plasma first wall
"""
function build_cx!(bd::IMAS.build, pr::Vector{Float64}, pz::Vector{Float64}, Z0::Float64)
    ipl = IMAS.get_build(bd, type=_plasma_, return_index=true)
    ...
    pr_lims, _ = IMAS.intersection(pr, pz, [0, maximum(pr) * 2.0], [Z0, Z0]; as_list_of_points=false)
    pr1 = sort(pr_lims)[1]
    pr2 = sort(pr_lims)[2]
