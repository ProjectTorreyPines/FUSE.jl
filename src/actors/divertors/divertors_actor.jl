...
=======
    ...
    if !_has_any_divertor(dd)
        @info "ActorDivertors: no divertors present; compute totals from SOL (no write-back)."
        write_divertor_totals_only_from_sol!(dd, sol1, λ_omp, actor.boundary_plasma_models, par)
    return actor
    end

    if !_has_any_target(dd)
        @info "ActorDivertors: divertors present but no targets; write totals only from SOL."
        write_divertor_totals_only_from_sol!(dd, sol1, λ_omp, actor.boundary_plasma_models, par)
        return actor
    end
