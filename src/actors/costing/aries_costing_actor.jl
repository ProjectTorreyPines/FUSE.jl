...
Base.@kwdef mutable struct FUSEparameters__ActorCostingARIES{T<:Real} <: ParametersActorBuild{T}
    ...
    _time::Float64 = NaN
    land_space::Entry{Real} = Entry{Real}("acres", "Plant site space required"; default=1000.0)
    building_volume::Entry{Real} = Entry{Real}("m^3", "Volume of the tokmak building"; default=140.0e3)
    interest_rate::Entry{Real} = Entry{Real}("-", "Annual interest rate fraction of direct capital cost"; default=0.05)
    indirect_cost_rate::Entry{Real} =
        Entry{Real}("-", "Indirect cost associated with construction, equipment, services, engineering construction management and owners cost"; default=0.4)
    escalation_fraction::Entry{Real} = Entry{Real}("-", "Yearly escalation fraction based on risk assessment"; default=0.05)
    blanket_lifetime::Entry{Real} = Entry{Real}("year", "Lifetime of the blanket"; default=6.8)
end
