...
function _step(actor::ActorGASCCosting)
    ...
    sub.cost = cost_direct_capital_GASC(:main_heat_transfer_system, power_thermal, da)
    total_direct_capital_cost += sub.cost

    #primary coils 
    sub = resize!(sys_fi.subsystem, "name" => "primary coils")
    sub.cost = cost_direct_capital_GASC(:primary_coils, cst, bd, da)
    total_direct_capital_cost += sub.cost

    #shields 
    sub = resize!(sys_fi.subsystem, "name" => "shields")
    sub.cost = cost_direct_capital_GASC(:shields, cst, bd, da)
    total_direct_capital_cost += sub.cost

    #structure 
    sub = resize!(sys_fi.subsystem, "name" => "structure")
    sub.cost = cost_direct_capital_GASC(:structure, cst, bd, da)
    total_direct_capital_cost += sub.cost

    #aux power 
    sub = resize!(sys_fi.subsystem, "name" => "aux power")
    sub.cost = cost_direct_capital_GASC(:aux_power, ec_power, ic_power, lh_power, nb_power, da)
    total_direct_capital_cost += sub.cost

    # blanket
    sub = resize!(sys_fi.subsystem, "name" => "blanket")
    sub.cost = cost_direct_capital_GASC(:blanket, par.capitalize_blanket, dd, da)
    total_direct_capital_cost += sub.cost

    #divertor
    sub = resize!(sys_fi.subsystem, "name" => "divertor")
    sub.cost = cost_direct_capital_GASC(:divertor, par.capitalize_divertor, dd, da)
    total_direct_capital_cost += sub.cost
    ...
    sub.cost = cost_direct_capital_GASC(:balance_of_plant, power_electric_net, power_thermal, da)
    total_direct_capital_cost += sub.cost

    #buildings 
    sub = resize!(sys_bld.subsystem, "name" => "buildings")
    sub.cost = cost_direct_capital_GASC(:buildings, bd, da)
    total_direct_capital_cost += sub.cost
    ...
    sys.yearly_cost = cost_fuel_GASC(:blanket, par.capitalize_blanket, dd, fixed_charge_rate, availability, plant_lifetime, neutron_flux, blanket_fluence_lifetime, power_electric_net, da)
    total_fuel_cost += sys.yearly_cost

    sys = resize!(cost_ops.system, "name" => "divertor")
    sys.yearly_cost = cost_fuel_GASC(:divertor, par.capitalize_divertor, dd, fixed_charge_rate, availability, plant_lifetime, neutron_flux, thermal_flux, divertor_fluence_lifetime, power_electric_net, da)
    total_fuel_cost += sys.yearly_cost

    sys = resize!(cost_ops.system, "name" => "aux power")
    sys.yearly_cost = cost_fuel_GASC(:aux_power, ec_power, ic_power, lh_power, nb_power, da)
    total_fuel_cost += sys.yearly_cost

    sys = resize!(cost_ops.system, "name" => "misc. fuel")
    sys.yearly_cost = cost_fuel_GASC(:misc_fuel, fixed_charge_rate, da)
    total_fuel_cost += sys.yearly_cost

    ###### Operations & Maintenance #####
    sys = resize!(cost_ops.system, "name" => "operations and maintenance")
    sys.yearly_cost = cost_operations_maintenance_GASC(power_electric_net, da)
    ...
    cst.levelized_CoE = 1e3 * (total_capital_cost * fixed_charge_rate + total_fuel_cost + cost_operations_maintenance_GASC(power_electric_net, da)) / (power_electric_net * 1e-6 * 8760 * availability) + 0.5e-3
...
function cost_direct_capital_GASC(::Type{Val{:main_heat_transfer_system}}, power_thermal::Real, da::DollarAdjust)
...
function cost_direct_capital_GASC(::Type{Val{:primary_coils}}, cst::IMAS.costing, bd::IMAS.build, da::DollarAdjust)
...
function cost_direct_capital_GASC(::Type{Val{:shields}}, cst::IMAS.costing, bd::IMAS.build, da::DollarAdjust)
    da.year_assessed = 2016
...
function cost_direct_capital_GASC(::Type{Val{:structure}}, cst::IMAS.costing, bd::IMAS.build, da::DollarAdjust)
    da.year_assessed = 2016
    primary_coils_hfs = IMAS.get_build_layer(bd.layer, type=IMAS._tf_, fs=_hfs_)
...
function cost_direct_capital_GASC(::Type{Val{:aux_power}}, ec_power::Real, ic_power::Real, lh_power::Real, nb_power::Real, da::DollarAdjust)
    da.year_assessed = 2016
...
#Equation 19
function cost_direct_capital_GASC(::Type{Val{:balance_of_plant}}, power_electric_net::Real, power_thermal::Real, da::DollarAdjust)
...
function cost_direct_capital_GASC(::Type{Val{:buildings}}, bd::IMAS.build, da::DollarAdjust)
    da.year_assessed = 2016
...
function cost_direct_capital_GASC(::Type{Val{:blanket}}, cap::Bool, dd::IMAS.dd, da::DollarAdjust)
    da.year_assessed = 2016
...
function cost_direct_capital_GASC(::Type{Val{:divertor}}, cap::Bool, dd::IMAS.dd, da::DollarAdjust)
    da.year_assessed = 2016
...
function cost_fuel_GASC(::Type{Val{:blanket}}, cap::Bool, dd::IMAS.dd, fixed_charge_rate::Real, availability::Real, plant_lifetime::Real, neutron_flux::Real, blanket_fluence_lifetime::Real, power_electric_net::Real, da::DollarAdjust)
    da.year_assessed = 2016
...
#Equation 24
function cost_fuel_GASC(::Type{Val{:divertor}}, cap::Bool, dd::IMAS.dd, fixed_charge_rate::Real, availability::Real, plant_lifetime::Real, neutron_flux::Real, thermal_flux::Real, divertor_fluence_lifetime::Real, power_electric_net, da::DollarAdjust)
    da.year_assessed = 2016
...
#Table III
function cost_fuel_GASC(::Type{Val{:aux_power}}, ec_power::Real, ic_power::Real, lh_power::Real, nb_power::Real, da::DollarAdjust)
    return 0.1 * cost_direct_capital_GASC(Val{:aux_power}, ec_power, ic_power, lh_power, nb_power, da)
end

function cost_fuel_GASC(::Type{Val{:misc_fuel}}, fixed_charge_rate::Real, da::DollarAdjust)
...
function cost_operations_maintenance_GASC(power_electric_net::Real, da::DollarAdjust)
    da.year_assessed = 1983
    if power_electric_net == 0
        cost = 49.1 # $M/yr 
    else
        power_electric_net = power_electric_net / 1e6
        cost = 7.7 * (power_electric_net / 1200)^0.5 # there is a typo in GASC 1986 equation F.10, which has been corrected here
    end
    return future_dollars(cost, da)
end