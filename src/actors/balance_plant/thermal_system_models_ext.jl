...
function _step(actor::ActorThermalSystemModels)
    dd = actor.dd
    par = actor.par
...
"""
    getval(v, act::ActorThermalSystemModels)
...
Inputs:
  ...
  - act::ActorThermalSystemModels

Outputs:

  - The actors default value for that parameter
"""
function getval(v, act::ActorThermalSystemModels)
    return act.var2val[act.sym2var[v]]
end

"""
    getvar(v,act::ActorThermalSystemModels)
...
"""
function getvar(v, act::ActorThermalSystemModels)
    return act.sym2var[v]
end

"""
    getsol(act::ActorThermalSystemModels)
...
"""
function getsol(act::ActorThermalSystemModels)
...
"""
    plant_wrapper(act::ActorThermalSystemModels; kw...)
...
"""
function plant_wrapper(act::ActorThermalSystemModels; kw...)
    return plant_wrapper(act.x, act.u, act.plant, act.optpar, act.var2val, act.sym2var; kw...)
end

"""
    plant_wrapper(act::ActorThermalSystemModels, yvars)
...
"""
function plant_wrapper(act::ActorThermalSystemModels, yvars)
    return plant_wrapper(act.x, act.u, yvars, act.plant, act.optpar, act.var2val, act.sym2var)
end

"""
    plant_wrapper(act::ActorThermalSystemModels, yvars, yfunc)
...
Ouput:
  ...
  - yfunc(soln.(yvars))
"""
function plant_wrapper(act::ActorThermalSystemModels, yvars, yfunc)
...
"""
function initddbop(act::ActorThermalSystemModels, soln::Nothing)
    return initddbop(act, act.gplot.gprops[:soln])
end

function initddbop(act::ActorThermalSystemModels, soln)
    gcopy = act.gplot
    dd = act.dd
...
"""
    setxATP!(x,actorATP::ActorThermalSystemModels)

Setter for actor.x
"""
function setxATP!(x, actorATP::ActorThermalSystemModels)
...
x0_idx are the indices in x0 which should be optimized, (x is just ActorThermalSystemModels.x)
"""
