...
#  ActorPowerNeeds  #
#= =============== =#
@actor_parameters_struct ActorPowerNeeds{T} begin
    model::Switch{Symbol} = Switch{Symbol}([:thermal_power_fraction, :EU_DEMO, :FUSE, :IFE], "-", "Power plant electrical needs model"; default=:FUSE)
...
Power needs actor that calculates the needed power to operate the plant
  ...
  - `model = :IFE` delegates to `_step_power_needs_ife!`, which is a no-op
    by default. Satellite packages (e.g. IFEdd) add a method dispatching on
    their own DD subtype to populate IFE driver wall-plug power.
