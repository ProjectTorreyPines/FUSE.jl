...
Base.@kwdef mutable struct FUSEparameters__ActorPowerNeeds{T} <: ParametersActor where {T<:Real}
    ...
    model::Switch{Symbol} = Switch(Symbol, [:gasc, :EU_DEMO, :FUSE], "-", "Power plant electrical needs model"; default=:FUSE) #thermal_power_fraction
    #bop_fraction::Entry{T} = Entry(T, "-", "Fraction of the gross electrical power generated by the thermal cycle (if model==`:thermal_power_fraction`)"; default=false)
...
function _step(actor::ActorPowerNeeds)
    ...
    if par.model == :gasc
        ...
        sys.power = par.bop_fraction .* bop.thermal_cycle.power_electric_generated
