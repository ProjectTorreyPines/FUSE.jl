...
function ActorEGGO(dd::IMAS.dd{D}, par::FUSEparameters__ActorEGGO{P}, act::ParametersAllActors{P}; kw...) where {D<:Real,P<:Real}
    ...
    return ActorEGGO(dd, par, act, green, basis_functions, NNmodel)
end
...
function _step(actor::ActorEGGO{D,P}) where {D<:Real,P<:Real}
    ...
    EGGO.get_surfaces(eqt, Matrix(transpose(psirz)), Ip, fcurrt, actor.green, wall, Rb_target, Zb_target, pp_target, ffp_target, ecurrt_target, Btcenter, Rcenter, pend)
