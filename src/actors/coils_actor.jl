...
"""
    init(pf_active::IMAS.pf_active, build::IMAS.radial_build, n_coils::Vector)
...
function init(pf_active::IMAS.pf_active,
    ...
    OH_layer = IMAS.get_radial_build(build, type=1)

    resize!(build.pf_coils_rail, length(n_coils))
    ...
    build.pf_coils_rail[1].name = "OH"
    build.pf_coils_rail[1].coils_number = n_coils[1]
    build.pf_coils_rail[1].coils_elements_area = coils_elements_area[1]
    build.pf_coils_rail[1].coils_cleareance = coils_cleareance[1]
    build.pf_coils_rail[1].outline.r = r_ohcoils
    build.pf_coils_rail[1].outline.z = z_ohcoils
    build.pf_coils_rail[1].outline.distance = range(-1, 1, length=n_coils[1])
    ...
    krail = 1
    resolution = 257
    rmask, zmask, mask = IMAS.structures_mask(build, resolution=resolution)
    for (k, layer) in enumerate(build.layer)
        if (layer.hfs == 1 || k == length(build.layer)) && ! is_missing(layer.outline, :r)
            if ! is_missing(layer, :material) && layer.material == "vacuum"
                ...
                build.pf_coils_rail[krail].name = replace(replace(layer.name, "hfs " => ""), "lfs " => "")
                build.pf_coils_rail[krail].coils_number = nc
                build.pf_coils_rail[krail].coils_elements_area = coils_elements_area[krail]
                build.pf_coils_rail[krail].coils_cleareance = coils_cleareance[krail]

                # pick layers with outline information
                if layer.hfs == 1
                    outer_layer = IMAS.get_radial_build(build, identifier=build.layer[k].identifier, hfs=1)
                    inner_layer = IMAS.get_radial_build(build, identifier=build.layer[k + 1].identifier, hfs=[1,0])
                else
                    inner_layer = IMAS.get_radial_build(build, identifier=build.layer[k - 1].identifier, hfs=1)
                    outer_layer = IMAS.get_radial_build(build, identifier=build.layer[k].identifier, hfs=[1,0])
                end
                ...
                if length(valid_k) == 0
                    build.pf_coils_rail[krail].outline.r = Real[]
                    build.pf_coils_rail[krail].outline.z = Real[]
                    build.pf_coils_rail[krail].outline.distance = Real[]
                ...
                # add rail info to radial_build IDS
                build.pf_coils_rail[krail].outline.r = valid_r
                build.pf_coils_rail[krail].outline.z = valid_z
                build.pf_coils_rail[krail].outline.distance = distance
...
function PFcoilsOptActor(eq_in::IMAS.equilibrium, build::IMAS.radial_build, n_coils::Vector; λ_regularize=1E-13, coil_model=:quick)
    # initialize coils location
    pf_active = IMAS.pf_active()
    init(pf_active, build, n_coils)
    ...
    # constructor
    pfactor = PFcoilsOptActor(eq_in, eq_out, time, pf_active, build, symmetric, λ_regularize, PFcoilsOptTrace(), coil_model)
...
# step
function pack_rail(build::IMAS.radial_build, λ_regularize::Float64, symmetric::Bool)::Vector{Float64}
    distances = []
    for rail in build.pf_coils_rail
        # not symmetric
        if ! symmetric
...
function unpack_rail!(optim_coils::Vector, packed::Vector, symmetric::Bool, build::IMAS.radial_build)
...
function optimize_coils_rail(eq::IMAS.equilibrium; pinned_coils::Vector, optim_coils::Vector, fixed_coils::Vector, symmetric::Bool, λ_regularize::Real, λ_ψ::Real, λ_null::Real, λ_currents::Real, build::IMAS.radial_build, maxiter::Int, verbose::Bool)
    ...
    packed = pack_rail(build, λ_regularize, symmetric)
    trace = PFcoilsOptTrace()

    packed_tmp = []
    function placement_cost(packed; do_trace=false)
        push!(packed_tmp, packed)
        λ_regularize = unpack_rail!(optim_coils, packed, symmetric, build)
...
function step(pfactor::PFcoilsOptActor;
    ...
    # run optimization
        ...
        build = pfactor.radial_build
        # run rail type optimizer
        if optimization_scheme in [:rail, :static]
            (λ_regularize, trace) = optimize_coils_rail(pfactor.eq_in; pinned_coils, optim_coils, fixed_coils, symmetric, λ_regularize, λ_ψ, λ_null, λ_currents, build, maxiter, verbose)
        else
