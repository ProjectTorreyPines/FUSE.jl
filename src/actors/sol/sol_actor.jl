...
@actor_parameters_struct ActorSOL{T} begin
    model::Switch{Symbol} = Switch{Symbol}([:box, :solps_nn, :replay, :none], "-", "SOL actor to run"; default=:box)
end
...
mutable struct ActorSOL{D,P} <: CompoundAbstractActor{D,P}
    ...
    SOL_actor::Union{Nothing,ActorSOLBox{D,P},ActorSOLPSNN{D,P},ActorReplay{D,P},ActorNoOperation{D,P}}
end
...
- `:solps_nn` - Uses ActorSOLPSNN edge-plasma surrogate (SOLPS-NN)
...
function ActorSOL(dd::IMAS.dd, par::FUSEparameters__ActorSOL, act::ParametersAllActors; kw...)
    ...
    elseif par.model == :solps_nn
        SOL_actor = ActorSOLPSNN(dd, act.ActorSOLPSNN)
