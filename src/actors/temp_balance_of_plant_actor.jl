...
function ActorBalanceOfPlant(dd::IMAS.dd, par::ParametersActor, act::ParametersActor; kw...)
    logging_actor_init(ActorBalanceOfPlant)
    par = par(kw...)
    IHTS_actor          = ActorIHTS(dd,act)
    thermal_cycle_actor = ActorThermalCYcle(dd,act)
    ActorBalanceOfPlant(dd, par, par.thermal_electric_conversion_efficiency,thermal_cycle_actor,IHTS_actor)
end
...
function _step(actor::ActorBalanceOfPlant; model::Symbol=actor.par.model)
    ...
    bop_thermal.thermal_electric_conversion_efficiency = actor.thermal_electric_conversion_efficiency
    bop_thermal.power_electric_generated = bop_thermal.cycle_net_work*actor.thermal_electric_conversion_efficiency
    ...
    bop.power_electric_net = bop_thermal.power_electric_generated-sys.power
    bop.Q_plant = bop.power_electric_net/sys.power
    return actor
end
