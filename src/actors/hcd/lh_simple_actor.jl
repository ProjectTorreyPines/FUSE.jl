...
#  LH  #
#= == =#
Base.@kwdef mutable struct FUSEparameters__ActorSimpleLH{T<:Real} <: ParametersActorPlasma{T}
...
function _step(actor::ActorSimpleLH)
    ...
    for (ps, lha) in zip(dd.pulse_schedule.lh.antenna, dd.lh_antennas.antenna)
        ...
        eta = par.ηcd_scale * TekeV * 0.037 * B0 / (5.0 + zeff) / ne20^0.33
