...
function case_parameters(::Type{Val{:ITER}}; init_from::Symbol, boundary_from=:MXH_params)::Tuple{ParametersAllInits,ParametersAllActors}
    ...
    ini.equilibrium.ip = (t -> ramp(t / 100.0) * 14E6 + ramp((t - 100) / 200.0) * 1E6) ↔ (2, (100.0, 300.0), (14E6, 15E6))
    ...
    ini.core_profiles.greenwald_fraction_ped = (t -> (ramp(t / 100.0) * 0.6 + ramp((t - 100) / 100.0) * 0.3) / 1.2) ↔ (2, (100.0, 300.0), (0.6/1.2, 0.9/1.2), (:match, :float))
    ...
    ini.core_profiles.zeff = (t -> 2.0) ↔ (2, (100.0, 300.0), (2.0, 4.0), (:match, :float))
